...
import { TMAP_APP_KEY } from '@/constant'
  ...
  setup(props, { emit }) {
    ...
    const setMarker = (location, isRedirectToCenter) => {
      ...
      if (!isEmpty(props.addressInfo)) {
        _infoWindow.value = new TMap.InfoWindow({
          ...
          <p style="color: #000;">
            地址: ${props.addressInfo.province}${props.addressInfo.city}${props.addressInfo.district || ''}
          </p>
        `, // 信息窗口内容
          offset: { x: 0, y: -50 }
        })
    ...
    onMounted(async () => {
      if (!window.TMap) {
        ...
        window[func] = initMap
        await loadScript(`https://map.qq.com/api/gljs?v=1.exp&libraries=service&key=${TMAP_APP_KEY}&callback=${func}`)
      } else {
        initMap()
      }
    })

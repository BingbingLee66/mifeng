<template>
  <div>
    <AModal :visible="props.entryVisible" :title="title" @ok="handleOk" @cancel="closeHandler" width="1000px">
      <div>
        <a-input v-model:value="keywordFilter.encyclopediaName" placeholder="请输入关键字" style="width: 340px">
          <template #prefix>
            <search-outlined style="color: #999" />
          </template>
        </a-input>
        <a-button type="primary" class="ml-4" @click="searchHandler()">搜索</a-button>
      </div>
      <EntryCard
        title="最近推荐"
        :lists="recommendList"
        ...
        @onChange="page => onChange(page, recommendFilter, queryRecommendList)"
        @onShowSizeChange="pageSize => onShowSizeChange(pageSize, recommendFilter, queryRecommendList)"
      ...
      <EntryCard
        ...
        @onShowSizeChange="pageSize => onShowSizeChange(pageSize, chamberFilter, queryChamberList)"
        @checkboxChange="checkboxChange"
      />
      <EntryCard
        ...
        @onShowSizeChange="pageSize => onShowSizeChange(pageSize, keywordFilter, searchList)"
        @checkboxChange="checkboxChange"
      />
    </AModal>
  </div>

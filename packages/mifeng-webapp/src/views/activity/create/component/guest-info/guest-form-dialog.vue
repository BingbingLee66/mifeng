<template>
  <a-modal
    ...
    @ok="onSave"
  >
    <a-form ref="formRef" :model="formState" :rules="rules" :label-col="{ style: { width: '150px' } }">
      <a-form-item label="嘉宾姓名：" name="name">
        <a-input v-model:value.trim="formState.name" placeholder="限15字内" :maxlength="15" clearable />
      </a-form-item>

      <a-form-item label="职位/头衔：" name="post">
        <a-input v-model:value.trim="formState.post" placeholder="限20字内" :maxlength="20" clearable />
      </a-form-item>

      <a-form-item label="所在公司/组织：" name="unit">
        <a-input v-model:value.trim="formState.unit" placeholder="限40字内" :maxlength="40" clearable />
      </a-form-item>
      ...
      <a-form-item label="嘉宾头像：" name="portrait">
        ...
        />
        <p style="margin: 0; padding: 0">建议上传尺寸600px*600px；小于300kb；.jpg,.gif,.png的图片</p>
      </a-form-item>
      ...
      <a-form-item v-if="showBank" label="保存至嘉宾库" name="isChamber">
        <a-checkbox v-model:checked="formState.isChamber" :disabled="disabledChamber">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    watch(
      ...
      val => {
        ...
        formState.value = {
          ...
          isChamber: chamberGuestsId ? chamberGuestsId !== '0' : !props.staticData.isChamber
        }
      }
    )
    ...
    const editChamberGuestPromise = async () => {
      ...
      emit('editChamber', { ...formState.value, isChamber: formState.value.isChamber ? 0 : 1 })
      onClose()
    }

  ...
  <div>
    ...
    >
      ...
      <a-table
        ...
        :row-key="record => `${record.id}:${record.name}:${record.post}:${record.unit}:${record.introduction}`"
        ...
        <template #bodyCell="{ record, column }">
          ...
          <template v-if="column.dataIndex === 'introduction'">
            <div>
              ...
              <div v-if="record.introduction && record.introduction.length >= 40">
                【<a-button type="link" @click="showText(record.introduction)">查看更多</a-button>】
              </div>
            </div>
          </template>
        </template>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const onConfirm = async () => {
      if (!selectData.value.length) return message.error(`请选择${ckey.value ? '会员' : '用户'}`)

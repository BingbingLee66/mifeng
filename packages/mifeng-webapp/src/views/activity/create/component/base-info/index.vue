<template>
  ...
  >
    ...
    <a-form-item v-if="applyObject.port" label="会内职位：" required>
      <a-select v-model:value="port" mode="multiple" placeholder="请选择" :disabled="status === 2 || status === 3">
        <a-select-option v-for="item in portSelect" :key="item.value" :value="item.value">
          {{ item.label }}
        </a-select-option>
      </a-select>
    </a-form-item>
    ...
    <a-form-item label="参加人数：" required>
      ...
      <a-checkbox v-model:checked="applyNum.limit" :disabled="status === 2 || status === 3" @change="handleCheckNum(1)">
        限
      </a-checkbox>
      <a-input
    ...
    <a-form-item label="扩展功能：">
      <a-checkbox-group v-model:value="expand">
        <a-checkbox :disabled="status === 2 || status === 3" :value="1">签到</a-checkbox>
        <a-checkbox :disabled="status === 2 || status === 3" :value="2">签退</a-checkbox>
        <a-checkbox :disabled="status === 2 || status === 3" :value="3">座位表</a-checkbox>
      </a-checkbox-group>
    </a-form-item>
    ...
    <a-form-item label="附件：">
      ...
      >
        <div class="import-upload-content">
          ...
          <div style="color: #999">上传格式限制word、excel、pdf、ppt、bmp、jpg、png、jpeg 大小限30M，限9个</div>
        </div>
      </a-upload>
    </a-form-item>
    <a-form-item label="活动类型：" name="labels" class="upload-style">
      <CustomSelect v-model:value="formState.labels" @click="activeVisible = true" style="width: 400px" />
    </a-form-item>

    <a-form-item label="活动介绍：" class="upload-style">
      <WangEditor v-model:content="formState.introduce" />
    </a-form-item>

    <div class="album-row">
      <a-form-item label="用户图片直播关联权限：">
        <a-radio-group v-model:value="formState.authAlbum" class="pt5" @change="onReleChange">
    ...
    <div v-if="ruleCkeys.includes(ckey) || !ckey">
      ...
      <a-form-item label="直播间链接：" name="link">
        <a-input
          ...
          style="width: 500px"
        />
        <a-select v-else v-model:value="formState.link" placeholder="请选择" allowClear style="width: 500px">
          <a-select-option v-for="item in audienceList" :key="item.id" :value="item.viewerUrl">
            {{ item.name }}
          </a-select-option>
        </a-select>
        <div class="mt10">
      ...
      <a-form-item v-if="formState.link && formState.link.trim()" label="入口显示：" name="liveCloseTime">
        <div>
          入口开放时间：
...
import TencentMap from '@/components/map/tencent'
...
import { getAudienceList } from '@/api/merchant/merchant'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const uploadHeadImage = async content => {
      ...
      const { data } = await uploadPortrait(formData, e => handleProgress(content, e))
    ...
    const beforeUploadFile = file => {
      ...
      if (
        ...
        proxy.$message.error('上传文件只能是 word、excel、pdf、ppt、bmp、jpg、png、jpeg格式!')
        return false
      }
      // 大小限制30M
    ...
    const handleRemoveAttachment = file => {
      formState.value.attachment = formState.value.attachment.filter(item => item.uid !== file.uid)
    }
    onMounted(() => {
      if (ckey.value) {
        treeSelectInit()
    ...
    return {
      ...
      expand,
      rules: {
        activityName: [{ required: true, validator: checkSpace, trigger: 'blur' }],

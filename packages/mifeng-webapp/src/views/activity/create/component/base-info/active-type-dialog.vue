<template>
  <a-modal :visible="visible" title="选择活动类型" :mask-closable="false" @cancel="onClose" @ok="onConfirm">
    <div class="tag-group-list">
      ...
      <a-pagination v-model:current="current" :page-size="5" :total="total" @change="handleTableChange" />
    </div>
  </a-modal>
</template>
...
import { useFetchTagList } from '@/views/activity/create/use/useFetchTagList'
  ...
  setup(props, { emit }) {
    ...
    watch(
      ...
      val => {
        if (val) {
          fetchData()
          checkList.value = props.activeType.map(v => `${v.labelId}:${v.label}`)
        }
      }
    )

<template>
  <Panel class="app-container">
    ...
    <a-tabs v-model:activeKey="activeName">
      ...
      <a-tab-pane key="2" tab="报名信息" :disabled="disabledApplyBtn">
        <ApplyInfo v-model:form="formState" v-model:apply-array="arrayData" :status="status" />
      </a-tab-pane>

      <a-tab-pane key="3" tab="活动嘉宾" :disabled="disabledGuestBtn">
        <ActiveGuest @guestList="addGuestList" />
      </a-tab-pane>
    </a-tabs>

    <div class="btn-group mt20">
      <a-form-item :wrapper-col="{ span: 14, offset: 4 }">
        <a-button v-if="activeName === '1'" @click="onPreview">预览</a-button>
    ...
    <!-- 到场人数提示 -->
    <a-modal title="到场人数设置后，小程序显示" v-model:visible="isPresent" width="30%">
...
import { useFetchDetail } from '@/views/activity/create/use/useFetchDetail'
import { useSubmitForm } from '@/views/activity/create/use/useSubmitForm'
  ...
  setup() {
    ...
    const validatorStepOne = async (showError = true) => {
      ...
      if (showError) {
        ...
        if (!formState.value.activityName || !formState.value.date.length || !formState.value.applyDate.length) {
          res = false
    ...
    const fetchAlbumRelevance = async () => {
      const { data, state } = await getAlbumRelevance({ activityId: activityId.value })
      if (!state) return
    ...
    const onNext = async () => {
      switch (activeName.value) {
        case '1':
          if (await validatorStepOne()) activeName.value = '2'
          break
        case '2':
          // 如果选择了自定义报名 但是没有选择自定义报名信息 就返回提示
          if (+formState.value.signType === 0 && !arrayData.value.length) {
            return message.error('自定义报名表需添加报名信息才可以发布活动，若无需自定义报名表，请选择【一键报名】')
          }
          activeName.value = '3'
          break
        default:
    ...
    const { onSave, ruleCkeys } = useSubmitForm(formState, roleIds, ckey, activityId, valueTree, portValue, arrayData)
    onMounted(() => {
      if (activityId.value) {
        fetchDetail()
        fetchAlbumRelevance()

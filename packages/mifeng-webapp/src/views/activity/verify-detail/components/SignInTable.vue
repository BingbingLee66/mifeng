<template>
  ...
  <a-modal v-model:visible="importVisible" title="导入" width="400px" :footer="null">
    ...
    <div class="flex-x-between-center mt-20">
      <a-button type="link" @click="onDownLoadSignin"> 下载导入表 </a-button>
...
function useTable({ id: activityId, activityStartTime, activityEndTime, activityName }, { activeKey, getStatusCount }) {
    ...
    if (now - start.value >= -1000 * 60 * 60 && now - end.value <= 0) {
      if (row.signStatus === 1) {
        ...
        return (
          <a-button
            type="link"
            onclick={() => signModalShow({ maxNum: row.subscribeTotal, num: row.subscribeTotal, siginId: row.id })}
          >
            签到
          </a-button>
        )
  ...
  const list = ref([])
  let total
  const tableFilter = reactive({ namephone: '', signStatus: -1, seatStatus: -1 })
        ...
        e.attachment.forEach(item => {
          item.signValue &&
            item.signValue.forEach(atta => {
              e.subAttr.push({ filename: atta.filename, url: atta.url, type: atta.type })
            })
        })
  ...
  const attachModal = reactive({ currentImg: '', imgDialog: false, showAttachment: false, currentRow: {} })
  ...
  const siginModal = reactive({ siginId: '', siginMaxnum: 0, siginNum: 0, signShow: false })
...
function useRemark(list) {
  const remarkModal = reactive({ remarkShow: false, remarkValue: '', remarkId: '' })
...
function useUpload({ id: activityId }, fetchTableData) {
  ...
  const onDownLoadSignin = () => {
    downloadFile({
      title: '报名信息模板表.xlsx',
      url: `${process.env.VUE_APP_BASE_API}/ecservice/api/ec/singin/download/dynamicTemplate/${activityId}`
    })
  }

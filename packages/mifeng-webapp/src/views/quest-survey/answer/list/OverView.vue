...
<script setup>
import { getAnswersListByMiF, answersOtherDetailByMiF, answersDetailOhter, downloadByMiF } from '@/api/quest-survey'
...
watchEffect(
  async () => {
    ...
    const { data } = await getAnswersListByMiF({ businessType: 1, questionnaireId })
...
const handleAnswerDetail = async (index, record) => {
  if ([COMPONENT_KEY.SINGLE_TEXT, COMPONENT_KEY.PROVINCE_CITY_AREA].includes(record.componentKey)) {
    ...
    // 下载附件
    try {
      const result = await downloadByMiF(record.problemId)
...
const showOtherDetail = async record => {
  ...
  const { data } = await answersDetailOhter({ questionId })

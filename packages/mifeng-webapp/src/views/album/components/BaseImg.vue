<template>
  <div class="img-item" @click="$emit('click', $event)">
    ...
    <a-tag v-if="tag.name" class="img-tag" :color="tag.type" @click.stop="$emit('tagClick')">{{ tag.name }}</a-tag>
    <slot />
    <div class="flex-x" @click.stop>
      ...
      />
      <a-button v-if="operations.includes('preview')" type="link" @click="previewVisible = true">查看大图</a-button>
      <a-button v-if="operations.includes('cover') && editAuth" type="link" @click="setCover">设为封面</a-button>
    </div>
  </div>
</template>

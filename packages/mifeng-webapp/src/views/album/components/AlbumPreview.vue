<template>
  <div v-show="visible" class="overlay" @click.stop="$emit('update:visible', false)">
    <div class="preview">
      ...
      <div class="content">
        ...
        <div class="info">
          ...
          <div v-if="cutStr" class="album-intr" :class="{ expanded: toggle }">
            {{ toggle ? intro : cutStr }}
            <div class="expand" @click.stop="toggle = !toggle">{{ toggle ? '收起' : '展开' }}</div>
          </div>
      ...
      <a-button class="footer" type="primary" @click="$emit('update:visible', false)">回到相册创建</a-button>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const bgImg = `url(${useImgUrl('commons/phone.png')}) no-repeat center / contain`
    return {
      dateStr,
      timeStr,
      imgList,

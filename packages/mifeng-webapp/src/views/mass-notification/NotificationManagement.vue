    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex === 'sendTs'">
          {{ SEND_TYPE_MAP.get(record.sendType) + dayjs(+record.sendTs).format('YYYY-MM-DD hh:mm:ss') }}
        </template>
        ...
        <template v-if="column.dataIndex === 'action'">
          ...
          <a-button
            type="link"
            primary
            @click="openUnread(record)"
            v-if="record.resendAuth === UNREAD_SEND_STATUS.ABLE"
          >
            未读重发
          </a-button>
        </template>
...
const tableColumns = [
  ...
  { title: '接收人类型', dataIndex: 'receiveTypeId' },
  { title: '接收人数', dataIndex: 'receiverNum' },
  { title: '已读/未读人数', dataIndex: 'readNum' }, // unreadNum
  { title: '已读率', dataIndex: 'readRate' },

  ...
  >
    <template #bodyCell="{ column, record }">
      ...
      <template v-if="column.dataIndex === 'readNum'">
        <div v-if="record.noticeTypeId === 7">--</div>
        <div v-else>
          <a-button
      ...
      <template v-if="column.dataIndex === 'readRate'">
        {{ format(multiply(bignumber(record.readRate), bignumber(100)), { precision: 2 }) + '%' }}
      </template>
...
<script setup>
import { ref, reactive, computed, defineAsyncComponent, inject } from 'vue'
import { format, bignumber, multiply } from 'mathjs'
...
const ReceiverDialog = defineAsyncComponent(() => import('../ReceiverDialog'))
const MassReadInfoDialog = defineAsyncComponent(() => import('../MassReadInfoDialog'))
const UnreadDialog = defineAsyncComponent(() => import('../UnreadDialog.vue'))
...
const handlerSearch = value => {
  const { noticeTypeId, chamberName, publishTime, title } = value

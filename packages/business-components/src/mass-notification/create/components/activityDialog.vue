...
import { ref, reactive, onMounted, computed, watch, getCurrentInstance, inject } from 'vue'
...
import { getInfoList, getInvesActivityList } from '@business/common/src/attract/api'
...
const fetchFn = async pageOption => {
  ...
  if (ckey.value && props.activityType === 2) {
    query.ckey = ckey.value
  }
...
const onChange = (selectedRowKeys, selectedRowItems) => {
  rowSelection.selectedRowItems = [selectedRowItems[selectedRowItems.length - 1]]
  rowSelection.selectedRowKeys = [selectedRowKeys[selectedRowKeys.length - 1]]
}
const rowSelection = useRowSelection({ hideSelectAll: true, onChange })

const { proxy } = getCurrentInstance()
const onSubmit = () => {
  if (!rowSelection.selectedRowKeys.length) return proxy.$message.warning('请选择活动')
  console.log(rowSelection.selectedRowItems[0])

  ...
  const params = computed(() => {
    ...
    const obj = {
      ...
      channelTypeTemplateDTOS: synchChannels.map(item => {
        if (item === NOTIFICATION_TYPE.FIVEG - 1) return { channelTypeId: item, id: activityChannels[0] }
    ...
    switch (receiveTypeId) {
      case RECEIVER_TYPE.MEMBER_SELF:
      ...
      case RECEIVER_TYPE.MEMBER_SPECIFY:
        obj.extend = { receiverList: receiverList.map(item => ({ id: item })) }
        break
      case RECEIVER_TYPE.DESIGNATE_SELF:
        obj.extend = { receiverList: receiverList.map(item => ({ id: item.ckey })) }
        break
      case RECEIVER_TYPE.PHONE_CUSTOM:
        obj.extend = { receiverList: receiverList.map(item => ({ id: item })) }
        break
      case RECEIVER_TYPE.CLERK_BACKGROUND:
        obj.extend = {
          receiverList: receiverList.map(item => ({ id: item.phone })),
          selectMemberList: receiverList,
          imgs: activityData.qrCode,
          title: introduceTitle,
          content: introduce
        }
        break
      case RECEIVER_TYPE.CLERK_FRONT:
        obj.extend = {
          receiverList: receiverList.map(item => ({ id: item.phone })),
          selectMemberList: receiverList
        }

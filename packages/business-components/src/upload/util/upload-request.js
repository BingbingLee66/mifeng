...
import { APP_CODE } from "@business/common/src/config";
import { checkFile } from "@business/common/src/content/api/article";
...
export default async function upload(option) {
  ...
  const apiServer =
    httpsHost + "/api/ecservice/upload/richhtml-custom-img-upload";
  /*  if (ckey.value) {
    formData.append("folder", "mifeng");
    apiServer = httpsHost + "api/ecservice/ec/upload/file2name";
  } */
  ...
  axios
    ...
    .then((res) => {
      ...
      option.onSuccess(
        { url: res.data.data.url, fileName: res.data.data.filename },
        res
      );
    })

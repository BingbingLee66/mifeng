<template>
  <div class="guest-wrap">
    <div class="header-wrap">
      ...
      <a-button class="mr10" @click="guestBankVisible = true"
        >从嘉宾库选</a-button
      >
        ...
        <template v-if="column.dataIndex === 'introduction'">
          <div>
            ...
            <div v-if="record.introduction && record.introduction.length >= 60">
              【<a-button type="text" @click="showText(record.introduction)">
                查看更多 </a-button
              >】
            </div>
          </div>
    ...
    <MemberBankDialog
      :static-data="tableData"
      v-model:visible="memberBankVisible"
      @confirm="addStaticData"
    />
...
import { getGuestList } from "@business/common/src/activity/api/activity-guest";
  ...
  setup(props, { emit }) {
    ...
    const editStaticData = (val, type) => {
      const findIndex = tableData.value.findIndex(
        (v) => (type === "chamber" ? v.chamberGuestsId : v.id) === val.id
      );

    ...
    >
      ...
      >
        ...
        <div v-if="+infoData.info === 0">
          ...
          >
            <a-input
              v-model:value="formData.msgAlert"
          ...
          <a-form-item label="输入字数限制" name="lengthLimit">
            <a-input
              v-model:value="formData.lengthLimit"
              placeholder="不限制"
              type="number"
            />
            <br />不填写，则默认不限制
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const addOptions = () => {
      ...
      if (formData.value.selects.length >= 10)
    ...
    const onInfoData = () => {
      ...
      if (+infoData.info === 0 || +infoData.info === 1 || +infoData.info === 2)

  ...
  <a-form :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
    <a-form-item label="报名方式：">
      <a-radio-group v-model:value="formState.signType">
        <a-radio :disabled="status_startingOrEnd" :value="0"
          >自定义报名表</a-radio
        >
        <a-radio :disabled="status_startingOrEnd" :value="1">一键报名</a-radio>
      </a-radio-group>
    </a-form-item>

    <a-form-item label="报名资料：">
      <a-radio-group
        v-model:value="formState.signNeedCard"
    ...
    >
      <div style="display: flex; flex-direction: column" class="pt5 pb5">
        ...
        >
          ...
          <a-radio :value="2">{{
            ckey ? "限本商会会员可见" : "限商会会员可见"
          }}</a-radio>
          <a-radio :value="3">限符合报名条件的用户可见</a-radio>
          <a-radio :value="4"
            >限报名成功的用户可见 (若需审核，指审核通过的用户)</a-radio
          >
        </a-radio-group>
      </div>
    </a-form-item>
    ...
    <a-form-item
      v-if="formState.signNeedCard === 1 && formState.cardShow === 1"
      label="名片墙信息设置："
    >
      <a-radio-group
        v-model:value="formState.cardInfoType"
        :disabled="status_startingOrEnd"
      >
    ...
    <a-form-item label="是否支持代报名">
      <a-radio-group
        v-model:value="formState.canReplaceApply"
        :disabled="status_startingOrEnd"
      >
    ...
    <div class="sgin-up">
      ...
      <div class="sgin-way">
        ...
        <a-radio-group v-model:value="formState.arriveType">
          <a-radio :disabled="status_startingOrEnd" :value="1">需填写</a-radio>
          <a-radio :disabled="status_startingOrEnd" :value="0"
            >无需填写</a-radio
          >
        </a-radio-group>
    ...
    <div v-show="+formState.signType === 0" class="mt20">
      <div style="display: flex">
        ...
        <a-button
          type="primary"
          :disabled="arrayData.length >= 20 || status_startingOrEnd"
          @click="iscustom = true"
        >
          +自定义报名信息
        </a-button>
      ...
      <div v-for="(item, index) in arrayData" :key="item.id" class="ml30">
        <a-form-item :name="'col' + index" :required="item.check === 1">
          ...
          <div class="flex flex-x-start-center">
            ...
            <div
              class="sign-right"
              v-if="index > 1 || formState.canReplaceApply !== 1"
            >
              <a-button type="link" @click="edit(index, item.type)"
                >编辑</a-button
              >

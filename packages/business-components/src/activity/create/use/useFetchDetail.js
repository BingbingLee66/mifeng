...
import { getActivity } from "@business/common/src/activity/api/activity";
// 获取活动详情
    ...
    getActivity({ id: activityId.value }).then((res) => {
      ...
      if (resData.startTime && resData.endTime) {
        formState.value.date = [
          formatDate(resData.startTime),
          formatDate(resData.endTime),
        ];
      }

      if (resData.applyStartTime && resData.applyEndTime) {
        formState.value.applyDate = [
          formatDate(resData.applyStartTime),
          formatDate(resData.applyEndTime),
        ];
      }
      ...
      formState.value.liveCloseTime = dayjs(
        Number(resData.liveEntranceCloseTime)
      ...
      arrayData.value = resData.dtos.map(
        ({ title, msgAlert, lengthLimit, check, type, selects, key }) => ({
          title,
          msgAlert,
          lengthLimit,
          check,

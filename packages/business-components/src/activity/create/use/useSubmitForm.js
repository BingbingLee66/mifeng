...
import { PUBLISH_TYPE } from "@business/common/src/activity/constant";
...
export function useSubmitForm(
  ...
  const onSave = throttle(async (e, lastPublishStatus) => {
    ...
    if (formState.value.date && formState.value.date.length > 0) {
      formState.value.activityStartTime = new Date(
        formState.value.date[0]
      ).getTime();
      formState.value.activityEndTime = new Date(
        formState.value.date[1]
      ).getTime();
    } else {
    ...
    if (formState.value.applyDate && formState.value.applyDate.length > 0) {
      formState.value.applyStartTime = new Date(
        formState.value.applyDate[0]
      ).getTime();
      formState.value.applyEndTime = new Date(
        formState.value.applyDate[1]
      ).getTime();
    } else {
    ...
    if (
      !(ruleCkeys.value.includes(ckey.value) || !ckey.value) ||
      !formState.value.link?.trim()
    ) {

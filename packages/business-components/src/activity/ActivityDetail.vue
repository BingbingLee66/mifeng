<template>
    ...
    <a-card class="board" style="width: 60%">
      <div class="flex-x-between-center">
        <div class="board-left">
          ...
          <div class="board-item">
            <span class="mr10">举办地点</span> {{ activity.addressInfo }}
          </div>
        ...
        <div class="board-right">
          ...
          <div class="qr-code-desc">
            ...
            <a-button
              type="link"
              @click="
                downloadFile({ url: activity.qrCode, title: '活动二维码' })
              "
              >下载</a-button
            >
          </div>
        ...
        <div class="flex-x-between-center">
          ...
          >
            <img
              ...
              @click="
                showImage({ url: activity[item.codeKey], title: item.title })
              "
            />
            <a-button
              type="link"
              @click="
                downloadFile({ url: activity[item.codeKey], title: item.title })
              "
  ...
  <Panel>
    <a-row justify="space-between">
      ...
      <a-col :span="6"
        ><div class="grid-content">
          ...
          <div class="value">
            {{ activity.auditStatus === 1 ? "需审核" : "不需审核" }}
          </div>
        </div></a-col
      >
      <a-col :span="6">
    ...
    <a-row justify="space-between">
      ...
      <a-col :span="6">
        <div class="grid-content">
          ...
          <div class="value">
            {{ $filters.dateFormat(activity.applyStartTime) }} ~
            {{ $filters.dateFormat(activity.applyEndTime) }}
          </div>
        </div></a-col
      >
    </a-row>
    <a-row justify="space-between">
      <a-col :span="6">
        <div class="grid-content">
          ...
          <div class="value">
            {{ activity.isLimit == 0 ? "不限" : activity.activityCount }}
          </div>
        </div></a-col
      >
      <a-col :span="6">
        <div class="grid-content">
          ...
          <div class="value">
            <div v-if="activity.labels" class="tags">
              <div
                v-for="(item, index) in activity.labels"
                :key="index"
                class="tags-block"
              >
                #{{ item.label }}
                <span v-if="activity.labels.length - 1 != index">,</span>
              </div>
            </div>
          </div>
        </div></a-col
    ...
    <a-row justify="space-between">
      ...
      <a-col :span="6"
        ><div class="grid-content">
          ...
          <div class="value">
            {{ activity.zhiboAddressType == 1 ? "云会播小程序" : "H5链接" }}
          </div>
        </div></a-col
      >
      <a-col :span="6">
    ...
    <div class="grid-content">
      ...
      <div class="value">
        <div v-for="(item, index) in activity.attachment" :key="index">
          <a-button type="link" @click="downLoadAttach(item)">{{
            item.fileName
          }}</a-button>
        </div>
      </div>
    ...
    <div
      v-if="activity.activityGuestsVOS && activity.activityGuestsVOS.length > 0"
      class="grid-content guests-content"
    >
      <div class="label">嘉宾信息：</div>
      <div class="value guests">
        <a-card
          v-for="item in activity.activityGuestsVOS"
          :key="item.id"
          class="box-card"
        >
          <div>
  ...
  <a-modal
    ...
    @cancel="handleCancel"
  >
    <img alt="二维码" style="width: 100%" :src="previewImage" />
  </a-modal>
</template>
<script>
import {
...
import { FILE_ACCEPT_TYPE } from "@business/common/src/activity/constant";
...
export default {
  ...
  setup(props) {
    const activityInfo = useActivity(props.activityId);
    return {
      ...activityInfo,
      codeList: [
...
function useActivity(activityId) {
  ...
  const expendValue = computed(() => {
    ...
    return (
      SIGN_IN_MAP.get(extraSignin) +
      SIGN_OUT_MAP.get(extraSignout) +
      SEATING_MAP.get(extraSeat)
    );
  });
  ...
  const downLoadAttach = (item) => {
    ...
    if (!type) return;
    if (FILE_ACCEPT_TYPE.indexOf(type) !== -1) {
      window.open(item.attachment);
    } else {
      downloadFile({ url: item.attachment, title: item.fileName });
    }
  };

...
import { perviewFile, downloadFile } from '@business/common/src/utils'
export default {
  props: {
    attachmentVisible: {
      type: Boolean,
      ...
      attachment.forEach(e => {
        if (e.signValue.length > 0) {
          e.signValue.forEach(f => {
            ...
            console.log(typeArr[typeArr.length - 1], FILE_ACCEPT_TYPE)
            if (FILE_ACCEPT_TYPE.indexOf(typeArr[typeArr.length - 1]) !== -1) {
              fileList.push(f)
            } else {
              imgList.push(f)
            }

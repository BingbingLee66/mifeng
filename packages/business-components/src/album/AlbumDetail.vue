      ...
      <div class="flex-x-between-center">
        <div class="flex-x-start-center">
          ...
          <a-select
            ...
            @change="
              onQueryChange({ auditStatus, imgId: '', isFinished: false })
            "
          />
        </div>
        <a-upload
          ...
          <div class="img-falls ml30">
            <a-timeline mode="left">
              ...
              >
                ...
                <BaseImg
                  :img="albumDetail.img"
                  :tag="generateTag({ useType: 1 })"
                />
              </a-timeline-item>
              <a-timeline-item
                v-for="date of imgFalls"
                color="gray"
                :key="date.name"
                type="primary"
              >
                    ...
                    <template v-for="img of date.children" :key="img.id">
                      ...
                      >
                        <template v-if="hasEditAuth">
                          <div
                            v-show="
                              !isCover(img) && selectedImgIds.includes(img.id)
                            "
                            class="img-overlay"
                          />
                          <close-circle-filled
                          ...
                          <i
                            ...
                            class="radio"
                            :class="{
                              selected: selectedImgIds.includes(img.id),
                            }"
                          />
                        </template>
                      </BaseImg>
              ...
              <a-timeline-item v-if="!imgFalls.length" color="yellow">
                ...
                <BaseImg
                  :img="{
                    img: 'https://ysh-cdn.kaidicloud.com/test/common/img_list_empty.png',
                  }"
                />
              </a-timeline-item>
            </a-timeline>
    ...
    <div class="footer flex-x-end-center">
      <template v-if="hasEditAuth">
        <slot
          ...
          :handleFreezeDisabled="handleFreezeDisabled"
          :handleOverFreezeDisabled="handleOverFreezeDisabled"
        ></slot>
        <!--  <a-button
          class="mr20"
      ...
      <template v-else>
        <a-button size="large" class="mr20" @click="toggleRelevance"
          >取消关联</a-button
        >
      </template>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const isFirstPerson = computed(() => {
      return +this.queryType === 0;
    });

    const handleFreezeDisabled = computed(() => {
      const hasOPEN = selectedImgs.value.some(
        (v) => v.status === PICTURE_TYPE_OPEN
      );
      const hasFreeze = selectedImgs.value.some(
        (v) => v.status === PICTURE_TYPE_FREEZE
      );
    ...
    const beforeUpload = (file) => {
      if (
        ...
        albumDetail.maxImgCount - albumDetail.imgCount
      ) {
        Message.error(
          `${file.name}上传失败,最多只能上传${albumDetail.maxImgCount}个文件`
        );
        return false;
      }
      if (!file.size) {
    ...
    const customRequest = ({ file }) => {
      ...
      uploadingList.value.unshift(
        formatImgData({ file, createdTs: Date.now(), id: `uid_${file.uid}` })
      );
    };
    // 按key将数组生成树
    ...
    const formatImgData = (img) => {
      ...
      const [dateStr, timeStr] = dayjs(+img.createdTs)
    ...
    const handleDownload = () => {
      if (!selectedImgIds.value.length)
        return Message.warning("请选择下载图片");
        ...
        selectedImgs.value.forEach(async (v) => {
          if (preDownloadingQueue.includes(v.id)) {
            ...
            try {
              ...
              const index2 = selectedImgs.value.findIndex(
                (img) => v.id === img.id
              ); // 下载完后将对应的选择框取消
        ...
        Modal.confirm({
          ...
          async onOk() {
            ...
            try {
              ...
              imgList.value = imgList.value.filter(
                (v) => !imgIds.includes(v.id)
              );
              selectedImgs.value = [];

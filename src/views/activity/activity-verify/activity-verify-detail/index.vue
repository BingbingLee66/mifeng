<template>
  <div class="app-container verify-container">
    <div style="height: 20px; margin-bottom: 20px">
      <span style="font-weight: 600; font-size: 22px; float: left"
        >活动报名审核</span
      >
    </div>
    <div class="statistic-wrap">
      <div class="intro">
        ...
        >
          活动来源：{{
            applyDetail.chamberName === null
              ? "凯迪云商会"
              : applyDetail.chamberName
          }}
        </div>
        <div>
          活动时间：{{ applyDetail.activityStartTime }} -
          {{ applyDetail.activityEndTime }}
        </div>
        <div>
          参加人数：
          ...
          <span v-if="applyDetail.isLimit === 1"
            >限{{ applyDetail.applyCount }}人
          </span>
        </div>
      </div>
      <div class="item-wrap">
        <div class="item-num">
          {{ applyDetail.appliedNum ? applyDetail.appliedNum : 0 }}
        </div>
        <div class="item-name">已报名人数</div>
      </div>
      <div class="item-wrap">
        <div class="item-num">
          {{ applyDetail.rejectedNum ? applyDetail.rejectedNum : 0 }}
        </div>
        <div class="item-name">审核驳回</div>
      </div>
      <div class="item-wrap">
        <div class="item-num">
          {{ applyDetail.approvedNum ? applyDetail.approvedNum : 0 }}
        </div>
      ...
      <div class="item-wrap">
        <div class="red-label">
          ...
          <span v-if="applyDetail.isLimit === 0">不限</span>
          <span v-if="applyDetail.isLimit === 1">{{
            applyDetail.applyCount - applyDetail.approvedNum
          }}</span>
        </div>
      ...
      <div class="item-wrap">
        <div class="item-num">
          {{ applyDetail.auditNum ? applyDetail.auditNum : 0 }}
        </div>
    ...
    <div style="margin: 10px 0" v-if="chamberName">
      ...
      <el-button
        ...
        size="small"
        type="warning"
        @click="handleMulAudit(2)"
        >驳回</el-button
      >
      <el-button size="small" type="text" @click="downloadSignTable"
        >下载签到表</el-button
      >
    </div>
          ...
          <template slot-scope="scope">
            ...
            <div>
              <el-button
                size="small"
                type="text"
                @click.native="showApplyDetailFunc(scope.row)"
                >查看全部</el-button
              >
            </div>
        ...
        <el-table-column label="审核状态" width="250px">
          <template slot-scope="scope">
            ...
            <div v-if="scope.row.auditStatus === 2">
              驳回理由：{{ scope.row.auditReason }}
            </div>
            <div class="blue-label" @click="showDetail(scope.row)">
              审核详情
            </div>
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            <div v-if="chamberName">
              ...
              <div
                ...
                "
              >
                <el-checkbox @change="(e) => handleSign(e, scope.row, 1)"
                  >已签到</el-checkbox
                >
                <br />
                <el-checkbox @change="(e) => handleSign(e, scope.row, 2)"
                  >未签到</el-checkbox
                >
              </div>
            </div>
    ...
    <el-dialog title="审核详情" :visible.sync="showDetailDialog" width="400px">
      <div
        ...
        v-for="(item, index) in applyDetailList"
        :key="item.id"
      >
        <div>
          {{ index === 0 ? "报名时间" : "再次报名" }}：{{ item.createdTs }}
        </div>
        ...
        <div v-if="item.auditStatus === 2">
          驳回理由：<span class="red-label">{{ item.auditReason }}</span>
        </div>
      </div>
      <div></div>
      <div slot="footer" style="text-align: center">
    ...
    <el-dialog title="提示" :visible.sync="showTipDialog" width="400px">
      <div v-if="applyCount - agreeCount === 0">
        <div>没有名额啦！</div>
        <div>
          该活动仅限 {{ applyCount }} 人参加，已有 {{ agreeCount }} 人审核通过
        </div>
      </div>
      <div v-else>
        该活动仅限 {{ applyCount }} 人参加，名额仅剩
        <span class="red-label">{{ applyCount - agreeCount }}</span> 个，
        允许审核通过的人数，需控制在 {{ applyCount - agreeCount }} 个以内哦
      </div>
      <div slot="footer" style="text-align: center">
        <el-button type="primary" @click="showTipDialog = false"
          >我知道了</el-button
        >
      </div>
    </el-dialog>
    ...
    <el-dialog title="查看全部" :visible.sync="showApplyDetail" width="400px">
      ...
      <div slot="footer" style="text-align: center">
        <el-button type="primary" @click="showApplyDetail = false"
          >了解了</el-button
        >
      </div>
    </el-dialog>

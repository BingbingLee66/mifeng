      ...
      <el-form ref="query" label-position="right" :inline="true" size="mini" :model="query">
        <el-form-item style="margin-right: 30px;" label="活动来源">
          <el-select v-model="query.invesKey" placeholder="请选择" clearable filterable>
            <el-option v-for="chamber in chamberOptions" :key="chamber.invesKey" :label="chamber.name" :value="chamber.invesKey" />
          </el-select>
        </el-form-item>
      ...
      <el-table v-loading="listLoading" :data="list" element-loading-text="Loading" border fit highlight-current-row>
        ...
        <el-table-column label="待审核" width="100px">
          <template slot-scope="scope">
            ...
            <div class="blue-label" @click="goVerifyDetail(scope.row,'auditStatus',0)">去审核</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" fixed="right">
          <template slot-scope="scope">
            <div class="blue-label" @click="goVerifyDetail(scope.row,'auditStatus',1)">录入签到情况</div>

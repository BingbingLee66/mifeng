  ...
  <div class="app-container">
    <div class="block">
      <el-form ref="query" label-width="auto" label-position="left" :model="query">
        <el-row>
          <el-col :span="6">
            <el-form-item label="会员名字/手机号码：">
              <el-input v-model="query.mulValue" />
            </el-form-item>
          </el-col>
          <el-col :span="5">
            <el-form-item :span="12" label="职位：">
          ...
          <el-col :span="5">
            <el-form-item :span="12" label="入会类型：">
    ...
    <br/>
    <el-row>
      <el-button type="primary" @click="add($event)" :actionid="getId('', '添加会员')" v-if="has('', '添加会员')">添加会员</el-button>
      <el-button type="primary" @click="exportExcel($event)" :actionid="getId('', '导表')" v-if="has('', '导表')">导表</el-button>
    </el-row>
    <el-table id="out-table" :data="list" v-loading="listLoading" element-loading-text="Loading" border fit highlight-current-row @selection-change="handleSelectionChange">
      ...
      <el-table-column label="行业" width="100px">
        <template slot-scope="scope">
          {{scope.row.tradeName}}
        </template>
      </el-table-column>

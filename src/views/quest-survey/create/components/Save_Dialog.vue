...
export default {
  ...
  methods: {
    ...
    async getQrCodeFunc(questionId) {
      ...
      const { data: { questionnaireCode, questionnaireUrl } } = await API({ questionnaireId: questionId })
      this.link = questionnaireUrl
      this.code = questionnaireCode
    },
    // 问卷发布
    async updateState() {
      const { questionId: questionnaireId, ckey, questionnaireTitle } = this
      const params = {
      ...
      if (res.state === 1) {
        ...
        // 去到《创建群发通知》页面
          self.$router.push({ path: '/mass-notification/create', query: { noticeType: 9, questionnaireId: questionnaireId || res.data, questionnaireTitle } })
        }).catch(() => {
        // 去到问卷管理页面

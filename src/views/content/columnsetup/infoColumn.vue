<template>
  <div class="app-container">
    <el-row>
      <el-button type="primary" @click="add($event)" :actionid="getId('', '新增栏目')" v-if="has('', '新增栏目')">新增栏目</el-button>
    </el-row>
    <el-table id="out-table" :data="list" v-loading="listLoading" element-loading-text="Loading" border fit highlight-current-row>
      ...
      <el-table-column label="操作" >
        <template slot-scope="scope">
          <el-button type="text" @click="openVisible($event, scope.row)" :actionid="getId('', '编辑')" v-if="has('', '编辑')">编辑</el-button>

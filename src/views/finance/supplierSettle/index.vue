<template>
  <div class="app-container">
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" border fit highlight-current-row @selection-change="handleSelectionChange">
      ...
      <el-table-column label="交易时间" width="120px">
        <template slot-scope="scope">
          {{scope.row.rptDateMin | dateFormat2}}至{{scope.row.rptDateMax | dateFormat2}}
        </template>
      </el-table-column>
      <el-table-column label="供货商" width="180px">
        <template slot-scope="scope">
      ...
      <el-table-column label="待结算金额(元)" width="150px">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" >
        <template slot-scope="scope">
          <el-button type="text" @click="confirm($event, scope.row)" :actionid="getId('', '确认结算')" v-if="has('', '确认结算') && scope.row.settlementStatus == 2" :disabled="scope.row.settlementStatus == 1">确认结算</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
